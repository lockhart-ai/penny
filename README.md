...
- **Active tab context** — Penny can see the page you're currently viewing (via [Defuddle](https://github.com/kepano/defuddle) article extraction, or — on a homepage or section front, which has no article to extract — the page's own list of headlines and where they point). Toggle "Include page content" to ask questions about any page
...
In the iOS app's Connection settings, tap **Send** beside **Test Notification** to force a test APNs notification to that registered device, even while the WebSocket is connected. The app sends a `test_notification` WebSocket message for this diagnostic.
