...
- **Model-facing change?** (prompt / `extraction_prompt` / tool description / what the model reads) → it MUST land with a `tests/eval/` contract, and you must **dry-run it against the live model** (`make eval` / focused case) and read the result *before* committing. Validate each lever as you build it, not batched at the end.
  ...
  - **Capture the run report in the PR body.** Set `EVAL_REPORT_DIR=/penny/data/eval-reports` on the `make eval` invocation (the target passes it through; `/penny/data` is the bind-mounted runtime dir — the same `./data` the DB lives in, already gitignored — so the markdown survives the `--rm` container without a source-tree artifact). The harness writes one `<case>.md` per case — a verbatim turn-by-turn transcript of every sample (👤 user · 🔧 Penny → tool · 📥 tool result · 🤖 Penny), with the structural verdict. Fold these into the **PR body**, one **`<details>` per case** (collapsed; the `<summary>` names the case + pass rate), under an always-visible summary table. This lets the reviewer read exactly what the model did on every run without a wall of text. The report is **never committed** — the PR body is the home; only the generator (in `tests/eval/conftest.py`) is committed.
    - **Re-runs after a lever change go as a PR COMMENT, not a body rewrite.** The body holds the *baseline* run; when you change a lever (a prompt / tool-response / description tweak) and re-run, post the new run as a **comment** rather than overwriting the body. Both survive, so the PR shows the before/after the amended commits produced — the evidence a lever was load-bearing.
