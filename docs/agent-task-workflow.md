...
- **Flag for the merge queue immediately after opening:** `gh pr merge <n> --auto --squash` ("merge when ready"). The PR then enqueues itself the moment it is approved and green — no one has to click merge. If the queue later kicks it back out (its `merge_group` checks failed against latest `main`), treat that exactly like red CI: diagnose, fix, push; the merge-when-ready flag survives and it re-enqueues.
...
8. **Flag merge-when-ready (`gh pr merge <n> --auto --squash`); the merge queue owns serialization** — rebase only on conflicts or a queue bounce, and never destructively escape a rebase.
