...
- **Pause only on a RESOLVED state — never while a check is pending.** Ending your turn "to wait for CI" on the strength of a background monitor is the classic dead-waiter: unless you have **verified** a live waiter process exists, nothing will ever wake you (four agents in one fleet parked this way on phantom waiters). While any check is pending, poll `gh pr checks` in-turn with short sleeps until it resolves; only the "green · current · awaiting merge" state (or an explicit supervisor-acknowledged block) is a legitimate place to stop.
