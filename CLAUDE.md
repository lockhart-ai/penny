...
- **Who approves splits on edges vs internals.** The code owner owns the **edges**: eval cases — their inputs (worlds, wordings, fixtures), their behaviour sentences, and the claims they assert. Adding, changing, porting, rewording or deleting a case on a new judgment goes to his review; the one exception is deleting a legacy case already ruled covered, which the supervisor approves. The supervising session owns the **internals**: Penny's runtime (prompts, machine, tools, stores, validators, plugins), the eval harness, CI and docs. It designs, reviews, approves and merges those itself, with the code owner's cases as the acceptance test. CODEOWNERS is the code owner alone, so the supervisor's approval is submitted under his own `gh` login, by his explicit choice; PR creation, comments and pushes stay on the App token. **Dispatched task agents never approve**, whatever the change. Still asked first: anything touching production (`make prod`/`up`/`kill`, the live DB), full-suite eval runs, and anything outward outside the repo
...
- **How** (top-level session, mirrors SOP §1): `git fetch origin main`, then `git worktree add -b <descriptive-branch> .claude/worktrees/<name> origin/main` (that path is gitignored). Do all editing, `make fix check`, and committing from **inside the worktree**. You are creating the worktree yourself, so `git worktree add` is correct — unlike a subagent dispatched with `isolation: "worktree"`, which is *already* in one and must not add another. **Then link the credentials in**, before anything in the new tree mints a token: replace its placeholder `.env` with a **symlink** to the primary checkout's (`ln -sfn /Users/decker/Documents/penny/.env .env`). Docker creates that placeholder as an empty *directory*, so the check is `[ -L .env ]` — **is it a symlink** — never `[ -e .env ]`, which the placeholder passes.
- **The SOP binds dispatched subagents, not the interactive session.** [`docs/agent-task-workflow.md`](docs/agent-task-workflow.md) is the operating contract for *task agents*: they cannot consult the user mid-task, so the document must carry the entire procedure, conservatively. An interactive session working directly with the user follows the **user's live instructions** instead, and adopts SOP steps only when the user says to. (Rationale: a session that treats the SOP as self-binding while *editing* it recursively binds itself to its own edits — this happened, and it produced ritual compliance like gating markdown diffs.) What an interactive session always keeps is the physics and the hard lines, not the process: worktree isolation (above), the token guard + push-before-`pr create` mechanics, the **privacy gate** (the repo is public no matter who edits), and the PR conventions (merge-when-ready flag; eval cases go to the code owner's review, internals are the supervisor's to approve — see Git Workflow). Docs-only diffs run no local gate — nothing in `make fix check` validates markdown; CI's required check covers branch protection.
- **`make token` gotcha:** without that symlink the worktree has no real `.env`, so mint against the primary checkout (`make -C /Users/decker/Documents/penny token`). With it, `make token` works in the worktree — which is what targets whose code must be the branch's, like `make eval-report`, require.
...
- **Ambient self-state and n≤1 reachability (the perception invariants)**: Penny's internal store, state, and logs are present in her ambient space — the self-state header renders her operational situation (active mechanisms, recent runs/mutations/autonomous sends, the store map, the durable user core) deterministically from the registry and ledger. Those logs and ledgers are *system-managed*: written by the framework at chokepoints (the prompt log, the mutation ledger, the outgoing-message log), never curated by the model. From that ambient state, **no piece of operational information is more than one general tool call away, and the call never requires a guess**: every anchor (a name, a typed id, a key) is rendered verbatim on a surface the model reads, and the small general verb set (`read_similar`, `log_read`, `memory_metadata`, `find`, …) consumes it. A required tool argument that no render emits is a reachability bug — guessing is n=∞ (in the motivating session, 17 of 21 tool errors were failed name guesses). Corollary: **metadata renders where the data renders** — a fact carried by a row the model already reads (provenance, status, origin) appears inline in that row's render, never behind a bespoke resolver verb; a resolver tool for a join the returned row already carries is a rendering bug wearing a tool costume (the `message_source` deletion on PR #1608 is the canonical counter-example), and the unmarked render case stays the quiet default. Enforcement is **design-time, on the ticket, by the ticket's designer**: any design touching a model-facing surface carries a *reachability sketch* — (a) the originating ambient state at the moment the information matters, and (b) the ≤1-call path (anchor → verb) from that state to each fact the design makes available — posted before implementation is dispatched. (Source: epic #1554's standing n≤1 invariant.)
- **A skill is an arbitrary tool sequence — never key logic to the example in front of you**: a skill is a *learned* sequence of tool calls, so nothing knows which tools it contains or in what order, and a plugin can introduce new ones at any time. Any rule keyed to a specific **tool name**, **argument position**, or the **phrasing of the failing sample** is wrong by construction, however well it fits the case that motivated it — it will simply not fire for a shape nobody enumerated, and by then it reads as intent rather than as the accident it was. The test before writing such a rule: *what happens when a tool I have never heard of does this?* If the answer is "not covered", the rule is keyed to the wrong thing. Prefer a **structural mark set once and read everywhere** over a name-set re-derived at each site; in model-facing text, name the **state or the principle**, never the verb ("configuring the routine is the turn" — not "don't browse", which fixes the browse sample and misses an email-only skill over-running identically). This governs fixtures too: an eval pool must cohere with the state it claims, and must never lend its own phrasing to the prompt under test. (Source: #1783 — a three-name write-tool whitelist that silently excluded `log_append` and every plugin tool's target; and #1706's apply instruction, whose first terminal condition described browsing rather than the state.)
- **The model's reasoning is ground truth (the rational-actor doctrine)**: the model acts rationally on the information it is given. When it does the wrong thing, the finding is a defect in the *presented state*, not in the model — fix the data so the correct choice is **self-evident** (an intent-carrying name, a visible parameter, a rendered anchor), never by stacking imperatives. Negative corrections ("NEVER X", "you MUST use Y") are guards for *edge cases* only; they are insufficient for promoting *core workflows* — a workflow the model must be ordered into is a workflow whose state presentation has failed. Diagnosis discipline: read the thinking trace as the model's honest account of the state it saw, and ask first "what did the state fail to present?" — a prompt imperative is the last rung of the fix ladder, not the first. (Source: the #1570 journey iterations — every dissected "failure" decoded as correct reasoning over deficient presentation: mechanism-named skills correctly read as not-watch-skills until descriptions carried the user's intent; an unbindable required parameter was correctly refused; a guard-blocked retry-after-remediation was correctly retried.)
...
- **SQLModel column expressions go through `col()`**: A SQLAlchemy operator on a model field is `col(Model.field).desc()`, never `Model.field.desc()` — ty resolves the bare attribute to the field's plain Python type, and `unresolved-attribute` is an **error** (a missing method is a real defect; the rule was a warning once and a deleted-method call hid in it for days). Never silence it with a per-line ignore where `col()` works: the suppression also hides the next real defect on that line. For the mapped table, `inspect(Model).local_table.c`
