...
penny/
  ...
  tools/
    ...
    memory_tools.py   — Tool subclasses over db.memories.*: reads are shape-specific and refuse the wrong shape — collections via collection_read_latest / collection_get / collection_read_random / collection_keys; logs via log_read only (no get; logs are streams). read_similar + memory_metadata are shape-agnostic. build_memory_tools(db, embedding_client, author) factory
  channels/
...
- 0059: Read-tool taxonomy rename in stored prompts — `read_latest(`→`collection_read_latest(`, `collection_metadata(`→`memory_metadata(` (shape-specific reads; collection reads error on logs and vice-versa)
- 0060: `collector-runs` becomes a read facade over `promptlog` — quality reviews runs via plain `log_read` (no `log_get`, no `penny-messages`); `notify` picks with `collection_read_random` (was newest-first + pick-one, which starved older thoughts); drop the dead `collector-runs` entry rows
- 0061: Drop the `user-messages`/`penny-messages` `memory_entry` replicas — they're read facades over `messagelog` now (the log marker rows stay)
