...
- System collections currently driven by collectors:
  ...
  - `skills` — **the COLLECTOR is RETIRED** (migration 0091, #1624); the COLLECTION stays active as the **teachable, user-editable standing-rules store** rendered ambiently in the self-state `### Skills and rules` section (#1471). The former reconcile collector (seeded 0043, regrounded 0069) read `collection_catalog` each cycle and folded *generalizable* recipe improvements into prose `skills` entries by model judgment. It existed to maintain prose build-recipes — the same authoring channel the structural path superseded: skills are now **taught** (`skill_create`, certified-by-execution, #1590), **instantiated** (`collection_create` renders a taught skill, #1591), **fired ambiently** (the standing-rules section, #1621), and **re-rendered** (`collection_update(skill=…)`, #1620). With no prose-generation step left to review, the model-judgment loop retired with the failure mode — the same disease `quality` had (0089) and the `notifier` tombstone before it (0086). Unlike those, the collection is **NOT archived** (its entries are the standing-rule feed the ambient section reads), so 0091 clears only the collector's dispatch fields (`extraction_prompt` + cadence) while the row stays active. 0091 also **pruned the two superseded build-recipe entries** (research-notify/silent) — they taught the model to hand-author a `collection_create` with a numbered `extraction_prompt`, the path #1591 replaced, so rendered ambiently they taught the wrong path; the operate-the-system standing rules (flip/cadence/archive/scope/one-shot, mute, email, likes/dislikes) STAY
...
- 0091: Retire the `skills` reconcile COLLECTOR; keep the COLLECTION as the standing-rules store (epic #1554, issue #1624) — clears the `skills` row's dispatch fields (`extraction_prompt` + the cadence columns) so the dispatcher never picks it up (`_is_ready` gates on `extraction_prompt IS NOT NULL`) and it drops out of the self-state Active-mechanisms section, while the row stays `archived = 0` so its entries keep rendering in the `### Skills and rules` section (#1471) and its store-map line survives. Also DELETEs the two superseded seeded build-recipe entries (`Research collection — notify on new finds` / `Research collection — silent`, 0069 lineage/known keys) — they taught the model to hand-author a `collection_create` with a numbered `extraction_prompt`, the path #1591's structural front door replaced. Operate-the-system standing rules (flip/cadence/archive/scope/one-shot, mute, email, likes/dislikes) STAY. Skills are now structural (taught #1590, instantiated #1591, fired ambiently #1621, re-rendered #1620), so a model-judgment loop maintaining prose recipes has no job — same cure as `quality` (0089) and `notifier` (0086). Universal (seeded row + two seeded entries by known key), idempotent
