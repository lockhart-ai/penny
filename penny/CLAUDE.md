...
penny/
  ...
  tools/
    ...
    memory_tools.py   — Tool subclasses: each funnels through `db.memory(name)` (the single dispatch) and calls a method on the returned `Memory` object, which refuses wrong-shape ops (collection ops on a log, log_read on a collection) via a base no-op (`WrongShapeError`) and read-only facades via `ReadOnlyMemoryError` — no tool branches on a name or shape. read_similar + memory_metadata are shape-agnostic; `find_mine` (#1558) resolves any object by meaning across the whole registry (collections + logs, archived included) + `skills` entries, plain-cosine, returning each hit's exact identity fused with the deterministic addressing (which tool operates on it) — the guess-free fallback every not-found error (MemoryNotFoundError) points at. `intent_inventory` / `intent_teardown` (#1559) are the "what did I set up for this?" pair: `intent_inventory(query | message_id)` enumerates everything one ask created as ONE archived-inclusive set — rows sharing a `source_message_id` (exact provenance) UNIONED with meaning-similarity over `intent` text (mechanisms set up across separate asks; thresholded at `MEMORY_INCLUSION_THRESHOLD`), each rendered with status + typed provenance ids and the exact teardown call; `intent_teardown(names=[...])` (lifecycle-tier, chat-only) cascade-archives that EXPLICIT enumerated list through the store's archive path, each independently (one failure never hides the siblings) with a per-item report. The two-tool shape IS the confirm-before-acting structure — enumerate, then act on exactly what was shown. build_memory_tools(db, embedding_client, author) factory
  channels/
