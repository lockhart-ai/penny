...
- **MessageLog**: Every user/agent message — `direction`, `sender`, `content`, `parent_id` (thread chain), `external_id` (platform ID), `is_reaction`, `thought_id` FK (notification source). **Emission provenance** (migration 0088, #1568): `mechanism` (the bound collection whose autonomous cycle produced an outgoing send — a by-name reference to `memory.name`, a plain indexed column like `mutation_event.entity_name`) + `novelty_key` (the emission's novelty identity). Both NULL for a direct reply (a chat turn with a live triggering user message is never novelty-gated and names no mechanism); the drainer stamps them at delivery time from the queued row. `why_did_i_send_that(message_id)` joins `mechanism` → the registry row → its spawning message, so "which mechanism sent this, and from which request?" is a read
...
- **SendQueueItem** (`send_queue` table): Durable outbound message queue — `content`, `collection` (the collector that queued it), `created_at`, `sent_at` (nullable; `NULL` = still pending, single source of truth — no separate flag). `send_message` enqueues here instead of dropping a message when the autonomous-send cooldown hasn't elapsed; the `SendQueueDrainer` delivers the oldest pending row once the cooldown clears. Kept after delivery (stamped `sent_at`) as an audit trail. **Novelty-keyed suppression** (migration 0088, #1568): `novelty_key` (the emission's novelty, computed in Python from the cycle's write-gate outcomes at enqueue time) + `suppressed_reason` (set on an emission the novelty gate held back — a repeat of the mechanism's last delivered novelty; NULL for a real emission). A suppressed row stays `sent_at IS NULL` but is excluded from the pending drain by its non-NULL `suppressed_reason` — a durable, datetime-ordered record of "we chose not to re-send this, and why". `next_pending`/`pending_items` filter `suppressed_reason IS NULL AND sent_at IS NULL`
...
- **Emission provenance + novelty-keyed suppression (#1568)**: every autonomous send names its mechanism end-to-end, and a mechanism is gated from re-sending the same news. **Provenance**: `send_message` enqueues with the bound collection (`send_queue.collection`); the `SendQueueDrainer` carries it (and the novelty key) onto the delivered `messagelog` row's `mechanism`/`novelty_key` at delivery (`send_response(mechanism=…, novelty_key=…)` → `_log_and_send` → `log_message`). A **direct reply** (a chat turn with a live triggering user message) stamps `mechanism = NULL` and is **never novelty-gated** — "is it still the same value?" always gets its answer. **Novelty key** is derived **in Python** at the send surface (enqueue time, where mechanism identity + the cycle's writes are in hand) from the entries this run wrote/changed — read structurally by the `last_written_by_run_id` stamp (`db.memories.entries_written_by_run`), never model-authored: `<collection>:<sha256-16 of the sorted key=content of the run's new/changed entries>`, or `<collection>:no-change` when the cycle wrote nothing new. **Suppression** compares against the mechanism's most recent real emission (`send_queue.latest_novelty_key`, pending or delivered, suppressed rows excluded): identical → `send_message` records the emission as suppressed (`record_suppressed`, `mutated=False`, an actionable receipt naming the unchanged key) and does NOT enqueue; changed or no prior → enqueue (`"Message sent."`, `mutated=True`). This is **layer 2** at the channel — the write-gate STOP (`KEY_EXISTS_UNCHANGED`) is layer 1 (same-cycle, same-key no-change, before the notify steps); layer 2 catches what STOP can't (a mechanism minting a fresh entry key, or a dedup-rejected write, for the same news). The outcome is a closed `EmissionOutcome` enum (`ENQUEUED`/`SUPPRESSED`). **Read side**: `why_did_i_send_that(message_id)` (a chat-only read tool, rides the lifecycle tier in `build_memory_tools`) renders `Sent by '<mechanism>' (every <cadence>), created <when> from your message: "<ask>"` or `Direct reply in conversation.`. Recent autonomous sends also join the **self-state header's activity block** (`sent · <when> · <mechanism> — "<snippet>"`, interleaved with runs + mutations; direct replies excluded). Kept THIN: #1580's `get_event` will likely absorb `why_did_i_send_that` as its send case.
...
- 0088: Emission provenance + novelty-keyed suppression columns (epic #1554, issue #1568) — `messagelog.mechanism` + `messagelog.novelty_key` (the mechanism that sent an autonomous message and the novelty it was gated on; both NULL for a direct reply) and `send_queue.novelty_key` + `send_queue.suppressed_reason` (the emission's novelty and, if held back, why). Schema only, universal, idempotent. Backs the reverse index — emission → mechanism — so "make it stop" is a lookup: `why_did_i_send_that`, the novelty gate in `send_message`, the drainer's provenance stamping, and the self-state activity block's `sent · …` lines
