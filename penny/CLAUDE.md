...
- 0085: Add `memory.notify` (epic #1554, issues #1591/#1557, stage ⑤ of #1562) — the emission-as-property flag (default 0), seeded from `published` (a column-wide copy) so a collection that already notified keeps notifying. `notify` supersedes the `published` pub/sub side-channel but both coexist: the reworked `collection_create` mirrors `notify` into `published` so the live notifier keeps delivering in the interim, until #1557 retires that machinery and wires the run-time notify suffix off `notify`
