...
penny/
  ...
  database/
    ...
    media_store.py    — MediaStore: browsed images, attached at egress to a reply that cites their source page (generated images deliver by id, not through here)
...
- Image generation: when an image model is configured, the `generate_image` tool is registered (mirroring the retired `/draw` command's conditionality). It generates the image via `OllamaImageClient`, stores it in the `media` table (no embedding), and returns a text result naming what was drawn plus the stored row's id on `ToolResult.media_id`. The loop threads that id onto `ControllerResponse.generated_media_ids` and the channel attaches **exactly that row** to the mirror-back reply at egress (`send_response(media_ids=...)`) — a deterministic generate→deliver link, not a fuzzy match, so a just-drawn image always lands on the reply that describes it (nothing travels through the model)
...
- **Media**: Images captured while browsing or drawn on request, delivered side-channel — `mime_type`, `data` (raw bytes), `source_url`, `title`, `embedding` (of title+URL; browsed rows only). The browse tool stores every page image here; at channel egress `MediaStore.select_image` attaches a **cited-page** image, most-relevant-first: (1) **exact URL** — if the outgoing message links a page we captured, that page's own image (newest capture); (2) **same domain** — else the embedding-nearest image from a domain the message links. Both deterministic; a reply that cites no source gets no image (the old tier-3 embedding-nearest jitter was removed in #1564 — it shipped random images onto ordinary messages). A `generate_image` row carries no `source_url`/`embedding` and is **not** matched here: it is delivered by id to its own reply (`send_response(media_ids=...)`, from `ToolResult.media_id`). Zero model involvement — no `<media:ID>` tokens, no prompt changes
...
- **Image side-channel**: Images never travel through the model. Two delivery paths, both deterministic. **Browsed images** — the browse tool decodes each page's image (base64 data URI from the extension), stores the bytes in the `media` table with an embedding of the page title+URL, and the agent loop carries no attachments. At egress (`send_response`), the outgoing text is embedded once (reused for the `penny-messages` log) and `MediaStore.select_image` attaches a **cited page's own capture** only — exact URL → same domain, both deterministic. A reply that cites no source gets **no image** (there is no embedding-nearest jitter fallback — that fired random images onto ordinary messages, #1564). **Generated images** — the `generate_image` chat tool stores the drawn image (no embedding) and stamps the row's id onto `ToolResult.media_id`; the loop threads it onto `ControllerResponse.generated_media_ids` and `send_response(media_ids=...)` fetches **exactly that row** by id and attaches it to the reply. This is the deterministic generate→deliver link that replaced fuzzy-matching a just-drawn image against the whole media table (which gave it only ~1-in-K odds of being the one attached). This whole scheme replaced a model-carried `<media:ID>`/inline-URL token scheme that couldn't reliably thread image references through multi-page replies
