...
def _score_searched(token: str):
    ...
    def score(db, before, reply) -> list[str]:
        if not tool_was_called(db, _SEARCH_EMAILS):
            return ["did not call search_emails for an email lookup"]
        args = last_tool_args(db, _SEARCH_EMAILS) or {}
        blob = " ".join(str(v) for v in args.values()).lower()
        if token not in blob:
            return [f"search args {args!r} dropped the token {token!r}"]
        return []

    return score


def _score_no_email(db, before, reply) -> list[str]:
    """Grumbling about email volume must NOT trigger any email tool."""
    fired = sorted(name for name in _EMAIL_TOOLS if tool_was_called(db, name))
    if fired:
        return [f"email tool(s) {fired} fired on a casual mention of email"]
    return []
