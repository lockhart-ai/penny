...
No elicit round precedes these -- the user's first message already carries the steps, so the
turn both learns and runs the round in one pass. The world is the composed one, which knows
neither the page nor the fact, so everything the round reports has to come from the
demonstration it was just given.
"""
