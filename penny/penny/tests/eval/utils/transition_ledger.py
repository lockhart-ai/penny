...
A transition case starts mid-conversation, so the turns before its beat have to already be in the store -- the classifier draws that parked it, the chat steps, the asks and replies, the tool calls and their results.  These are the writers that put them there, in production's own shapes, so a seeded history is indistinguishable from a lived one.

Shared machinery: the transition cases build every world with it, and so do the collector, speakable-log-read and memory-story worlds.
"""
