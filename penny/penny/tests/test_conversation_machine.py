...
def test_render_idle_slice_whole():
    ...
    assert render_classifier_content(_IDLE_SNAPSHOT, _ASK) == (
        ...
        "\n"
        "## Current state\n"
        "idle — ordinary conversation: chat, questions, passing mentions, anything put off "
        "for later, and every request handled right now in the conversation — including "
...
def test_render_parked_elicit_slice_whole():
    ...
    assert render_classifier_content(_ELICIT_SNAPSHOT, _STEPS) == (
        ...
        "\n"
        "## Current state\n"
        "elicit — the user wants something set up to keep running on its own, no known "
        "skill covers it, and the assistant is asking to be taught the steps\n"
        "\n"
        "## Transitions\n"
