...
def test_render_parked_learn_with_candidates_whole():
    ...
    assert render_classifier_content(taught, "yeah, check it every morning") == (
        ...
        "— a schedule, a repeat, or accepting an offer to set it up — and their message "
        "changes no STEP of the routine: how often it runs, until when, and whether it "
        "tells them are the job's terms, not steps; a message that fixes or adds a step "
        "(what to read, what to look for, what to remember, where to save it) is "
        "instructions, even when it also sounds like a yes — add a second line naming "
        "that skill: SKILL: <its name, copied exactly from Known skills>\n"
        "- learn — the user's message is a set of instructions to follow for the task "
        "being worked on — what to read, what to look for, what to remember, including "
        "corrections to previous steps\n"
        "- idle — in all other cases"
