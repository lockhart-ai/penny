...
def test_system_prompt_whole_render():
    ...
    assert STATE_CLASSIFIER_SYSTEM_PROMPT == (
        ...
        "the user's newest message puts the conversation in. The assistant has real "
        "tools (reading pages, saving values), and a separate context carries out "
        "whatever is decided — never judge whether an action is possible; your only "
        "job is the state. Respond with exactly "
        "one line:\n"
        "STATE: <name>\n"
...
def test_render_parked_elicit_slice_whole():
    ...
    assert render_classifier_content(_ELICIT_SNAPSHOT, _STEPS) == (
        ...
        "\n"
        "States:\n"
        "- learn: their message answers the teach question — it names what to read, "
