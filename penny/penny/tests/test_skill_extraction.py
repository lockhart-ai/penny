...
def test_naming_system_prompt_whole_render():
    ...
    assert SKILL_NAMING_SYSTEM_PROMPT == (
        ...
        "belongs in that spot each time the routine runs. Wording it is not deciding "
        "it: an instruction whose target the user named is theirs however you phrased "
        "it — 'the admission fee listed on the museum's visitor page' for their \"look "
        "up the museum's admission fee\" is the user's value; what you produced is only "
        "the sentence.\n"
