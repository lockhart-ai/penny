...
_BASIC_FLOW_EXPECTED = (
    ...
    "yourself RIGHT NOW with exactly that. A round ends with the WRITE: read the "
    "sources, pull the items, then collection_write what you found "
    "(found-but-not-written teaches nothing). Then mirror back what you did. Don't "
    "ask them to restate it as steps; confirm by doing.\n"
    "   b. You genuinely can't tell what one round should do — ask for the missing "
    "piece in ONE message (you learn from one complete pass), then run the round "
    "the moment they answer.\n"
    "Either way you'll learn it automatically as a skill from that round — then "
