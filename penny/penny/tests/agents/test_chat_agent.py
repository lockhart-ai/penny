...
_BASIC_FLOW_EXPECTED = (
    ...
    "misses, three cases:\n"
    "   a. Their request already tells you what one round needs — where to look, "
    "what to pull out, what to keep — then that IS the routine: run one round "
    "yourself RIGHT NOW with exactly that. If the target collection doesn't exist "
    "yet, create it (name + description, nothing else) and keep going. An edge "
    "you're unsure about (does X count?) is not a blocker: take your best "
    "reading, run the round, and say what you included — they'll correct you if "
    "you guessed wrong (case c). A round ends with the WRITE: read the sources, "
    "pull the items, then collection_write what you ACTUALLY found — the real "
    "titles and values from the pages, never an invented sample "
    "(found-but-not-written teaches nothing; invented data teaches a lie). Then "
    "mirror back what you did. Don't ask them to restate it as steps; confirm by "
    "doing. Writing the routine out in your reply is not running it: the moment "
    "you catch yourself listing its steps or offering a plan for confirmation, "
    "make those steps as tool calls instead — the reply comes after the write.\n"
    "   b. Where to look or what to pull is genuinely missing from everything "
    "they've said — ask them for the full routine, start to finish, in a single "
    "message ('give me the whole routine in one message — I'll do it right away; "
    "I learn from one complete pass'), then run the round the moment they answer. "
    "A missing or fuzzy schedule is never this case: the round itself has no "
    "schedule.\n"
    "   c. They say a round you ran got it wrong — ask for the complete corrected "
    ...
    "the skill appears when the round closes (your reply ends it), so attach it "
    "NEXT turn with collection_update(name=<slug>, skill=<its name>, "
    'trigger="every <seconds>", notify=…), never mid-round. Cadence words in '
    "their ask ('morning and evening', 'daily') are the trigger for that attach "
    "step, not part of the round — don't design the trigger until the skill "
    "exists.\n"
