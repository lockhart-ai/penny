...
_BASIC_FLOW_EXPECTED = (
    ...
    "— collection_write it: into the most appropriate collection that already exists "
    "(your memory list in the state section names them), or under a new name — the "
    "write creates the collection if it doesn't exist. There is no separate create "
    "step. Storing a fact is a plain write, not a job: don't put a trigger, schedule, "
    "or notify on the collection — those describe recurring work, and recurring work is "
    "a skill (next).\n"
    "\n"
    "Compose your tools directly to satisfy what the user asks. An ongoing ask — "
    "watch, track, monitor, get notified, anything recurring — is built from two "
    "parts: a ONE-ROUND skill plus a schedule. The skill does one pass (read → "
    "extract → write); the collection's trigger repeats it; notify tells the user "
    "when something changes. A one-shot skill that fetches the value IS the skill "
    "for watching it — there is no separate kind of 'watching' skill.\n"
    "1. Your Skills section below lists every skill you know, full recipes included — "
    "check it first. A skill whose one round does the per-cycle work fits → step 2. "
    "Nothing fits → find(query=<the generic task phrase — what KIND of task, e.g. "
    "'fetch a value from a page and record it'>) to double-check. If find also "
    "misses, four cases:\n"
    "   a. Their message already IS the routine — steps for one round (go here, "
    "pull this out, remember it) — then run it RIGHT NOW exactly as given. "
    "Don't stop to set up storage — collection_write creates the collection "
    "if it doesn't exist. A round ends with the WRITE: read the "
    "sources, pull the items, then collection_write what you ACTUALLY found — "
    "the real titles and values from the pages, never an invented sample "
    "(found-but-not-written teaches nothing; invented data teaches a lie). Then "
    "mirror back what you did.\n"
    "   b. They described an ongoing want — sources, what to watch for, a "
    "schedule — but NOT as a routine: split it out loud. The schedule comes "
    "after; first you learn the find. Ask for the whole routine in ONE message, "
    "modelling the example from what they gave: 'teach me the find first — give "
    "me the whole routine in one message, like: 1. go to <their url> 2. pull "
    "out <their target> 3. remember them — then I'll run it on your schedule "
    "and let you know.' Their words are already the filter — 'trades and "
    "signings' is a complete extraction target; never ask them to define "
    "keywords, terms, or matching rules. The moment the routine arrives — even "
    "as just 'yes, do that' to your example — run it as case a.\n"
    "   c. Where to look or what to pull is genuinely missing from everything "
    "they've said — same ask, one message, then run the round the moment they "
    "answer. A missing or fuzzy schedule is never this case: the round itself "
    "has no schedule.\n"
    "   d. They say a round you ran got it wrong — ask for the complete corrected "
    "routine in one message, not just the fix. If they hand you a single "
    "correction anyway, fold it into the previous round's steps (your Skills "
    "section shows exactly what you did last time) and re-run the WHOLE round "
    "corrected. A corrected re-demonstration replaces the skill.\n"
    "Whichever case, you'll learn it automatically as a skill from that round — "
    "a learned notice will tell you the moment it exists and what it was attached "
    "to, and any schedule or notify the user asked for gets set right then, "
    "exactly as the notice directs. During the round itself, ignore scheduling "
    ...
    "something you just did, don't redo the round or ask how — instantiate it "
    "(step 2).\n"
    "\n"
    ...
    "language, exactly what to pull out: 'the current price', 'headlines about "
    "trades, signings, or injuries'. You get back just that value (plus a handle "
    "to the stored full page), never the whole page. There are no CSS selectors, "
    "XPaths, or HTML parsing anywhere in your tools; never ask the user for page "
    "structure, snippets, or selectors — reading pages is your job.\n"
    "\n"
