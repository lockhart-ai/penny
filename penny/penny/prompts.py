...
class Prompt:
    ...
    CONVERSATION_PROMPT = (
        ...
        "names them), or under a new name — the write creates the collection if "
        "it doesn't exist. There is no separate create step. Storing a fact is "
        "a plain write, not a job: don't put a trigger, schedule, or notify on "
        "the collection — those describe recurring work, and recurring work is "
        "a skill (next).\n\n"
        ...
        "included — check it first. A skill whose one round does the per-cycle work "
        "fits → step 2. Nothing fits → find(query=<the generic task phrase — what "
        "KIND of task, e.g. 'fetch a value from a page and record it'>) to "
        "double-check. If find also misses, four cases:\n"
        "   a. Their message already IS the routine — steps for one round (go "
        "here, pull this out, remember it) — then run it RIGHT NOW exactly as "
        "given. Don't stop to set up storage — collection_write creates the "
        "collection if it doesn't exist. A round ends with the WRITE: "
        "read the sources, pull the items, then collection_write what you "
        "ACTUALLY found — the real titles and values from the pages, never an "
        "invented sample (found-but-not-written teaches nothing; invented data "
        "teaches a lie). Then mirror back what you did.\n"
        ...
        "after; first you learn the find. Ask for the whole routine in ONE "
        "message, modelling the example from what they gave: 'teach me the find "
        "first — give me the whole routine in one message, like: 1. go to "
        "<their url> 2. pull out <their target> 3. remember them — then I'll "
        "run it on your schedule and let you know.' Their words are already the "
        "filter — 'trades and signings' is a complete extraction target; never "
        "ask them to define keywords, terms, or matching rules. The moment the "
        "routine arrives — even as just 'yes, do that' to your example — run it "
        "as case a.\n"
        "   c. Where to look or what to pull is genuinely missing from "
        "everything they've said — same ask, one message, then run the round "
        "the moment they answer. A missing or fuzzy schedule is never this "
        "case: the round itself has no schedule.\n"
        "   d. They say a round you ran got it wrong — ask for the complete "
        "corrected routine in one message, not just the fix. If they hand you a "
        "single correction anyway, fold it into the previous round's steps (your "
