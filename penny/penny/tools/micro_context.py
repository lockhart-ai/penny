...
SKILL_NAMING_SYSTEM_PROMPT = (
    ...
    "   A value that is itself an instruction — one saying what to look for or "
    "pull — is DESCRIBED as a plain-language instruction (e.g. 'the current "
    "price'): there is no CSS-selector, XPath, or pattern machinery in this "
    "system, so never name or describe one that way. How such a value is worded "
    "says nothing about who supplied it — the two cases above decide that.\n"
